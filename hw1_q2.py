# -*- coding: utf-8 -*-
"""
Created on Tue Mar 29 19:33:08 2022
...
def force(x, y):
    if fem.near(x, 0.3) and fem.near(y, 0.02):
        return (0, -100e3)
    else:
        return (0,0)
...
fem = FEM(dim, E, nu, cons="plane_stress")

mesh = read_mesh('ce522hw1q2.m')
p = mesh['POS']
t = mesh['QUADS'][:,:-1]
t = t -1
...
keep_nodes = np.array([0,3])

ndof=dim*(p.shape[0])

K, F = fem.assemble_all(p, t, force)

fixed=np.concatenate((2*keep_nodes, 2*keep_nodes+1))
